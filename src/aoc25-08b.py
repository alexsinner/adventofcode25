...
def compute_distances(junction_boxes):
    ...
    return sorted(distances, key=lambda x: x[2], reverse=False)
...
def compute_final_distance(distances, junction_boxes):
    all_boxes = set(junction_boxes)
    circuits = []
    ...
    for (jb1, jb2, _) in distances: 
        ...
        print(max([len(x) for x in circuits]))
        if max([len(x) for x in circuits]) == len(all_boxes):
            print(f"Found it: {jb1} x {jb2} = {jb1[0]*jb2[0]}")
            return jb1[0]*jb2[0]
...
#    junction_boxes = parse_input("data/input-08.txt")
    ...
    result = compute_final_distance(distances, junction_boxes)
    print(result)
